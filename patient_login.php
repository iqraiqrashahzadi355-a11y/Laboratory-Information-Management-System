...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT PatientID, Name, Age, Gender, ContactNumber FROM Patients WHERE PatientID = ?");
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $_SESSION['patient_contact'] = $patient['ContactNumber'];
        $stmt->close();
        header("Location: /LIMS/patient_dashboard.php");
        exit;
    } else {
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .id-input-wrap { display: flex; align-items: center; border: 1.5px solid #ddd; border-radius: 12px; overflow: hidden; margin-bottom: 1.25rem; background: #fafafa; transition: border-color 0.2s; }
        .id-input-wrap:focus-within { border-color: var(--accent); background: #fff; }
        .id-prefix { padding: 0 16px; font-size: 0.85rem; color: var(--muted); border-right: 1px solid #eee; height: 52px; display: flex; align-items: center; background: #f0f0f0; font-weight: 600; }
        .id-input-wrap input { flex: 1; border: none; background: transparent; padding: 0 16px; height: 52px; font-size: 1.3rem; font-family: 'Syne', sans-serif; font-weight: 700; color: var(--ink); outline: none; letter-spacing: 0.1em; }
        .id-input-wrap input::placeholder { color: #ccc; font-weight: 400; letter-spacing: 0; font-size: 1rem; }
...
<div class="brand-panel">
    ...
    <div class="features-list">
        ...
        <div class="feature-item"><div class="feature-dot"></div> Secure access with your unique Patient ID</div>
        <div class="feature-item"><div class="feature-dot"></div> Powered by Women University Multan LIMS</div>
    </div>
</div>
...
<div class="login-panel">
    <div class="login-box">
        <h2>Patient Login</h2>
        <p class="subtitle">Enter your Patient ID to access your records.</p>
